...
MeshShader::MeshShader(string vertexFilename, string fragmentFilename) : Shader(vertexFilename, fragmentFilename)
{
...
void MeshShader::setViewAndProjection(Camera *camera)
{
	...
	glUniformMatrix4fv(projMatLoc, 1, GL_FALSE, glm::value_ptr(camera->getProjectMatrix()));
	glUniformMatrix4fv(normalMatLoc, 1, GL_FALSE, glm::value_ptr(normalMat));
}
